...
def recomendar_presupuesto_dinamico(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if gasto_semana_anterior > presupuesto_semanal * 1.2:
        factor_ajuste = 0.90
        advertencias.append(f"La semana anterior superaste el presupuesto en S/ {gasto_semana_anterior - presupuesto_semanal:,.2f}. Presupuesto reducido 10%.")
...
def simular_escenario_que_pasaria_si(
    ...
    if delta_ahorro > 0:
        ...
        viabilidad = "Moderadamente viable — impacto leve"
...
def generar_reporte_lenguaje_natural(df: pd.DataFrame, periodo: str = "este período") -> Dict[str, Any]:
    ...
    if num_anomalias > 3:
        alertas.append(f"{num_anomalias} transacciones anómalas detectadas — revisión necesaria")
