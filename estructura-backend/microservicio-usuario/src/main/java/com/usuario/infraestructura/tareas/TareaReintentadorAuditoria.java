...
/**
 * Tarea programada (Scheduler) que reintenta publicar de forma polimórfica todos los eventos de auditoría
 * (Accesos, Transacciones y Auditorías Generales) pendientes en la bandeja de salida (Outbox).
 */
@Component
@Slf4j
    ...
    public void reintentarEventosPendientes() {
        ...
        for (BandejaSalidaAuditoria entrada : pendientes) {
            try {
                ...
                if (tipo.startsWith("ACCESO_")) {
                    // 1. Procesar Eventos de Acceso
                    EventoAccesoDTO evento = objectMapper.readValue(payload, EventoAccesoDTO.class);
                    String routingKey = "auditoria.acceso." + evento.estado().name().toLowerCase();
                    rabbitTemplate.convertAndSend(NombresExchange.AUDITORIA, routingKey, evento);
                    
                } else if ("TRANSACCIONAL".equals(tipo)) {
                    // 2. Procesar Eventos Transaccionales
                    ...
                    // 3. Procesar Eventos de Auditoría General
